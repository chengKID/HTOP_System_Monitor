...
long LinuxParser::ActiveJiffies(int pid) {
  string pid_string;
  string line;
  string tmp, nr14, nr15, nr16, nr17, nr22;
  ...
  if(filestream.is_open()){
    while (std::getline(filestream, line)) {
      ...
      while (linestream_proc >> tmp >> tmp >> tmp >> tmp >> tmp >> tmp >> tmp >> tmp >> tmp >> tmp >> tmp 
             >> tmp >> tmp >> nr14 >> nr15 >> nr16 >> nr17 >> tmp >> tmp >> tmp >> tmp >> nr22) {
        utime = std::stof(nr14);
        stime = std::stof(nr15);
...
long LinuxParser::ActiveJiffies(std::vector<std::string> vector_jiffies) {
  long active_jiffies_value = std::stof(vector_jiffies[0]) + std::stof(vector_jiffies[1]) +
                              std::stof(vector_jiffies[2]) + std::stof(vector_jiffies[5]) +
                              std::stof(vector_jiffies[6]) + std::stof(vector_jiffies[7]) +
...
string LinuxParser::User(int pid) {
  ...
  nr1_return = LinuxParser::Uid(nr1_return);
  return nr1_return;
}
...
long LinuxParser::UpTime(int pid) {
  ...
  while (std::getline(filestream_proc, line)) {
    std::istringstream linestream_proc(line);
    while (linestream_proc >> nr1 >> nr2 >> nr3 >> nr4 >> nr5 >> nr6 >> nr7 >> nr8 >> 
                              nr9 >> nr10 >> nr11 >> nr12 >> nr13 >> nr14 >> nr15 >> 
                              nr16 >> nr17 >> nr18 >> nr19 >> nr20 >> nr21 >> nr22) {
